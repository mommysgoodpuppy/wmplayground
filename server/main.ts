...
async function compileWorkman(source: string, stage: string = "all"): Promise<CompileResponse> {
  // Create temp file in WMC_ROOT so it's accessible via --dir flag
  const tempFileName = `temp_${Date.now()}_${Math.random().toString(36).slice(2)}.wm`;
  ...
  try {
    ...
    const fileExists = await Deno.stat(tempFile).then(() => true).catch(() => false);
    ...
    const grainArgs = [
      "--dir", WMC_ROOT.replace(/\\/g, "/"),
      "--include-dirs", join(WMC_ROOT, "src").replace(/\\/g, "/"),
    ...
    console.log("Command result:", { code, outputLength: output.length, errorLength: errorOutput.length });
...
const port = 3001;
console.log(`🗿 Workmangr Playground API running on http://localhost:${port}`);
